...
<template>
  <div class="flex flex-col h-full bg-[#1E2025]">
    ...
    <div class="flex items-center gap-3 border-b border-white/10 px-4 py-3">
      ...
      <h3 class="text-white text-base font-medium">历史记录</h3>
    </div>
    ...
    <div class="flex items-center justify-between px-4 py-3 border-b border-white/10">
      ...
      <div class="flex items-center gap-2">
        <select
          v-model="sortBy"
          class="px-3 py-2 bg-white/5 border border-white/10 rounded-2xl text-white text-xs focus:outline-none focus:border-[#00FFCC]/50 cursor-pointer"
        >
          <option value="date" class="bg-[#1E2025]">按日期排序</option>
          <option value="type" class="bg-[#1E2025]">按类型排序</option>
        </select>

        <button
          @click="handleDeleteExpired"
          class="px-3 py-2 bg-red-500/20 rounded-2xl text-red-400 text-xs font-medium hover:bg-red-500/30 transition-colors"
        >
          清理过期
        </button>
      </div>
    ...
    <div class="flex-1 overflow-y-auto px-4 py-4">
      ...
      <div v-else class="grid grid-cols-3 gap-3">
        <div
          ...
          :key="record.id"
          class="group relative rounded-2xl overflow-hidden bg-white/5 hover:bg-white/10 transition-all cursor-pointer"
        >
          <!-- 缩略图 -->
          <div class="relative aspect-video" @click="handlePreview(record)">
            ...
            <!-- 视频播放按钮 -->
            <div v-if="record.type === 'video'" class="absolute inset-0 bg-black/30 flex items-center justify-center">
            ...
            <div
              ...
              class="absolute top-2 right-2 px-2 py-1 bg-red-500/90 rounded-full text-white text-xs font-medium"
            >
              已过期
            </div>

            <!-- 类型标签 -->
            <div class="absolute top-2 left-2 px-2 py-1 bg-black/60 backdrop-blur-sm rounded-full text-white/90 text-xs font-medium">
            ...
            <div class="absolute bottom-2 right-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button
                ...
                class="p-1.5 bg-black/60 backdrop-blur-sm rounded-lg hover:bg-black/80 transition-colors"
                title="下载"
              >
              ...
              <button
                ...
                class="p-1.5 bg-red-500/80 backdrop-blur-sm rounded-lg hover:bg-red-500 transition-colors"
                title="删除"
              >
                <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="px-3 py-2">
            <div class="flex items-center justify-between mb-1">
              <span class="text-white/80 text-sm font-medium">分镜 #{{ record.shotNo }}</span>
              <span class="text-white/40 text-xs">{{ record.aspectRatio }}</span>
            </div>
            <p class="text-white/40 text-xs">{{ record.timestamp }}</p>
          </div>
        </div>
      </div>
    </div>
