...
func NewFFmpeg(log *logger.Logger) *FFmpeg {
	...
	// 检测 FFmpeg 和 FFprobe 路径
	ffmpegPath := detectFFmpegPath()
	ffprobePath := detectFFprobePath()

	log.Infow("FFmpeg paths detected", "ffmpeg", ffmpegPath, "ffprobe", ffprobePath)
...
// detectFFmpegPath 检测 ffmpeg 可执行文件路径
func detectFFmpegPath() string {
	// 1. 检查项目本地 bin 目录
	execPath, err := os.Executable()
	if err == nil {
		projectBin := filepath.Join(filepath.Dir(execPath), "bin", "ffmpeg")
	...
	wd, err := os.Getwd()
	if err == nil {
		wdBin := filepath.Join(wd, "bin", "ffmpeg")
...
// detectFFprobePath 检测 ffprobe 可执行文件路径
func detectFFprobePath() string {
	// 1. 检查项目本地 bin 目录
	execPath, err := os.Executable()
	if err == nil {
		projectBin := filepath.Join(filepath.Dir(execPath), "bin", "ffprobe")
		if _, err := os.Stat(projectBin); err == nil {
			return projectBin
		}
	}
	...
	wd, err := os.Getwd()
	if err == nil {
		wdBin := filepath.Join(wd, "bin", "ffprobe")
		if _, err := os.Stat(wdBin); err == nil {
			return wdBin
		}
	}

	// 3. 使用系统 PATH 中的 ffprobe
	return "ffprobe"
}
...
func (f *FFmpeg) downloadVideo(url, destPath string) (string, error) {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		if attempt > 1 {
			f.log.Infow("Retrying download", "attempt", attempt, "url", url)
		...
		if err != nil {
			lastErr = fmt.Errorf("failed to download (attempt %d): %w", attempt, err)
			f.log.Warnw("Download failed", "attempt", attempt, "error", err)
			continue
		}
		defer resp.Body.Close()

		if resp.StatusCode != http.StatusOK {
			lastErr = fmt.Errorf("bad status (attempt %d): %s", attempt, resp.Status)
			f.log.Warnw("Download bad status", "attempt", attempt, "status", resp.Status)
			continue
		}
		...
		if err != nil {
			lastErr = fmt.Errorf("failed to save file (attempt %d, written %d bytes): %w", attempt, written, err)
			f.log.Warnw("Download copy failed", "attempt", attempt, "written", written, "error", err)
			os.Remove(destPath) // 清理不完整的文件
			continue
		}

		f.log.Infow("Download successful", "attempt", attempt, "size", written, "dest", destPath)
		return destPath, nil
	}
