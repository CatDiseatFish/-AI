...
import (
	...
	pkgConfig "github.com/drama-generator/backend/pkg/config"
...
func (s *VideoMergeService) getVideoClient(provider string) (video.VideoClient, error) {
	...
	case "doubao", "volces", "ark":
		...
		// 检查全局配置中是否有 Doubao API Key 配置
		cfg, err := pkgConfig.LoadConfig()
		if err == nil && cfg.AI.Doubao.Enabled && cfg.AI.Doubao.APIKey != "" {
			// 使用配置文件中的 Doubao API Key
			return video.NewVolcesArkClient(
				cfg.AI.Doubao.BaseURL,
				cfg.AI.Doubao.APIKey,
				cfg.AI.Doubao.Model,
				cfg.AI.Doubao.Endpoint,
				cfg.AI.Doubao.QueryEndpoint,
			), nil
		}
		// 否则使用 Web 界面配置的 API Key
