...
func (c *VolcesArkClient) GenerateVideo(imageURL, prompt string, opts ...VideoOption) (*VideoResult, error) {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		if attempt > 1 {
			fmt.Printf("[VolcesARK] Retrying video generation (attempt %d)...\n", attempt)
		...
		if err != nil {
			lastErr = fmt.Errorf("send request (attempt %d): %w", attempt, err)
			fmt.Printf("[VolcesARK] Request failed (attempt %d): %v\n", attempt, err)
			continue
		}
		...
		if err != nil {
			lastErr = fmt.Errorf("read response (attempt %d): %w", attempt, err)
			fmt.Printf("[VolcesARK] Read response failed (attempt %d): %v\n", attempt, err)
			continue
		}
		...
		if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusCreated {
			lastErr = fmt.Errorf("API error (status %d, attempt %d): %s", resp.StatusCode, attempt, string(body))
			fmt.Printf("[VolcesARK] Bad status (attempt %d): %d\n", attempt, resp.StatusCode)
			// 如果是 4xx 错误，不重试
	...
	if lastErr != nil && (resp == nil || resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusCreated) {
		return nil, fmt.Errorf("video generation failed after %d attempts: %w", maxRetries, lastErr)
	}
